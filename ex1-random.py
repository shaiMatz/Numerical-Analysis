...
#Random funcion that adds 19 to 2 random numbers from my ID(checks that its between 0-11 -  12questions)
EladSapir = "209479948"
result=100
while result>=12:
    ...
    def is_triangular(self):
        ...
        for row in range(self.rows):
            ...
            if not all(self.lhs[x][self.leading[row]] == 0 for x in range(row + 1, self.rows)):
                if self.debug: print(
                    "Not all coefficients below the leading coefficient [{}, {}] are zero.".format(row + 1,
                                                                                                   self.leading[
                                                                                                       row] + 1))
                return False
            ...
            if not all(self.leading[row] > self.leading[x] for x in range(row)):
                if self.debug:
                    print(
                        "The leading coefficient at [{}, {}] is not to the right of all leading coefficients above.".format(
                            row + 1, self.leading[row] + 1))
                return False
        ...
        for i, col in enumerate(self.leading[::-1]):
            ...
            for row in range(i - 1, -1, -1):
                if self.lhs[row][col] != 0:
                    if self.debug:
                        print(self)
                        print("The leading coefficient at [{}, {}] is not the only one in its column! (Row {}).".format(
                            i + 1, col + 1, row + 1))
